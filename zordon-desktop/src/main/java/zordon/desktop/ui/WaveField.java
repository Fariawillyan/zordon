...
/** Renders the side waves, spectrum bars and playing-sample line. */
final class WaveField {

    private WaveField() {}

    static void draw(GraphicsContext g, VoiceVisualizer.WaveState s) {
        sideWaves(g, s);
        bars(g, s);
        sample(g, s);
        g.setStroke(color("#1CEBFF", 0.55));
        g.setLineWidth(1);
    ...
    private static void sideWaves(GraphicsContext g, VoiceVisualizer.WaveState s) {
        for (int wave = 0; wave < 26; wave++) {
            g.setStroke(color("#12DDF2", 0.14 + (wave % 5) * 0.05));
            g.setLineWidth(0.8);
            g.beginPath();
            boolean started = false;
            for (int x = 0; x <= (int) VoiceVisualizer.W; x += 3) {
                double distance = Math.abs(x - VoiceVisualizer.CX);
                if (distance < 150) { started = false; continue; }
    ...
    private static void bars(GraphicsContext g, VoiceVisualizer.WaveState s) {
        ...
        for (int side = 0; side < 2; side++) {
            for (int bar = 0; bar < 14; bar++) {
                ...
                g.setStroke(color("#1BE4F8", 0.35 + random.nextDouble() * 0.45));
                g.setLineWidth(1.4);
    ...
    private static void sample(GraphicsContext g, VoiceVisualizer.WaveState s) {
        if (!s.playing()) return;
        for (int glow = 2; glow >= 0; glow--) {
            g.setStroke(color("#21EAFF", glow == 0 ? 0.95 : 0.09));
